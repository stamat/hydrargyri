...
| `conditions`       | `Object`   | Named predicates for [`if` binds](bind.html#conditions) (`bind="items:if#isEmpty"`), called as `(value, element)` on every paint of the key — the initial `null` included. Truthy shows the node, falsy sets `hidden`. A missing condition warns and leaves the node as authored. Assignable at runtime: `el.conditions.isEmpty = fn`. |
