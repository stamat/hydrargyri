...
| `wires`            | `Object`   | Listeners the class wires itself, by selector — `{ 'audio, video': 'play:onPlay;pause:onPause' }`, the [same pair grammar `on` takes](on.html#static-wires), for the plumbing an element needs in every instance. Wired at scan on every matching node in scope, the element itself included; a pair the node's own `on` attribute already carries is skipped, so such markup keeps firing once. |
