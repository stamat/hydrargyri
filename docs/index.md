...
with a registry, which is also why hydrargyri is 4.0 kB gzipped where Stimulus
is 11.3. Past that split it reaches where the table above does not look: outlets
...
| [bind](docs/bind.html)               | where state lands: `text`, `html`, `value`, `attr#name`, `prop#name`, `class#name`, `if` / `unless`, and paths into objects |
